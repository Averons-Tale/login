...
function checkAuthAndRedirect() {
    auth.onAuthStateChanged(user => {
        if (user) {
            user.getIdToken().then(token => {
                ...
                if (window.location.hostname === 'login.nexorhub.com') {
                    window.location.href = 'https://community.nexorhub.com';
                }
            });
        } else {
            // Handle case when user is not logged in
            if (window.location.hostname === 'community.nexorhub.com') {
    ...
    function setCookie(name, value, days) {
        ...
        document.cookie = `${name}=${value};${expires};path=/;domain=nexorhub.com;Secure;SameSite=None`;

        firebase.auth().currentUser.getIdToken(true).then(function(idToken) {
            document.cookie = `auth_token=${idToken};path=/;domain=.nexorhub.com;secure`;
            window.location.href = 'https://community.nexorhub.com';
        });
    }

    function deleteCookie(name) {
        document.cookie = `${name}=; path=/; domain=nexorhub.comt; expires=Thu, 01 Jan 1970 00:00:00 GMT; Secure; SameSite=None`;
    }
...
function syncAuthState() {
    auth.onAuthStateChanged(user => {
        if (user && window.location.hostname === 'login.nexorhub.com') {
            user.getIdToken().then(token => {
                setCookie('auth_token', token, 30);
                window.location.href = 'https://community.nexorhub.com';
            });
